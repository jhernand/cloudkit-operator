...
func (r *ClusterOrderReconciler) handleHostedCluster(ctx context.Context, instance *v1alpha1.ClusterOrder,
	...
	err := r.Client.List(ctx, nodePools, client.MatchingLabels{
...
func (r *ClusterOrderReconciler) handleNodePool(ctx context.Context, instance *v1alpha1.ClusterOrder,
	...
	// Find the matching item inside the `nodeCounts` field of the status, or create a new one if there is no
	// matching item yet.
	resourceClass := instance.Spec.NodeRequests[0].ResourceClass
	var nodeRequestStatus *v1alpha1.NodeRequest
	for i, nodeCountsItem := range instance.Status.NodeCounts {
		if nodeCountsItem.ResourceClass == resourceClass {
			nodeRequestStatus = &instance.Status.NodeCounts[i]
		}
	}
	...
	// Update the selected `nodeCounts` item:
...
func (r *ClusterOrderReconciler) handleNoHostedCluster(ctx context.Context, instance *v1alpha1.ClusterOrder) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)
